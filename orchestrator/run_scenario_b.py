...
class ScenarioRunner:
    def __init__(self, args):
        ...
        self.run_id = args.run_id or f"scenario-b-{int(self.run_started_at)}"
    ...
    def run(self):
        self._record(
            "run_start",
            {
                "run_id": self.run_id,
                "version": "B",
                "tests": ["anomaly-hot-partition", "anomaly-compaction-pressure"],
            },
        )
        ...
        # Test 1: Hot partition anomaly.
        self._post(self.learner_base, "/phase", {"phase": "chaos"})
        hot_body: Dict = {
            "profile": "anomaly-hot-partition",
            "duration_sec": self.args.hot_sec,
            "cpu_workers": self.args.cpu_workers,
            "mem_mb": self.args.mem_mb,
            "replicas": self.args.bottleneck_replicas,
        }
        if self.args.hot_threads is not None:
            hot_body["threads"] = self.args.hot_threads
        hot_body["parallel_jobs"] = self.args.hot_parallel_jobs
        hot_start = self._post(self.chaos_base, "/start_fault", hot_body)
        self._record("test1_start", {"profile": "anomaly-hot-partition", "start": hot_start})
        # Keep a fault_start/fault_stop pair for at least one chaos window (reporting uses the last pair).
        self._record("fault_start", hot_start)
        self._sleep_phase(Phase("test1_anomaly_hot_partition", self.args.hot_sec))
        hot_stop = self._post_chaos_stop_fault("anomaly-hot-partition")
        self._record("test1_stop", {"profile": "anomaly-hot-partition", "stop": hot_stop})
        self._record("fault_stop", hot_stop)

        self._post(self.learner_base, "/phase", {"phase": "cooldown"})
        self._sleep_phase(Phase("cooldown_after_test1", self.args.cooldown1_sec))

        # Test 2: Compaction pressure anomaly.
        self._post(self.learner_base, "/phase", {"phase": "chaos"})
        comp_body: Dict = {
            "profile": "anomaly-compaction-pressure",
            "duration_sec": self.args.compaction_sec,
            "cpu_workers": self.args.cpu_workers,
            "mem_mb": self.args.mem_mb,
            "replicas": self.args.bottleneck_replicas,
        }
        if self.args.compaction_threads is not None:
            comp_body["threads"] = self.args.compaction_threads
        if self.args.compaction_pop_end is not None:
            comp_body["pop_end"] = self.args.compaction_pop_end
        comp_body["parallel_jobs"] = self.args.compaction_parallel_jobs
        comp_start = self._post(self.chaos_base, "/start_fault", comp_body)
        self._record("test2_start", {"profile": "anomaly-compaction-pressure", "start": comp_start})
        self._record("fault_start", comp_start)
        self._sleep_phase(Phase("test2_anomaly_compaction_pressure", self.args.compaction_sec))
        comp_stop = self._post_chaos_stop_fault("anomaly-compaction-pressure")
        self._record("test2_stop", {"profile": "anomaly-compaction-pressure", "stop": comp_stop})
        self._record("fault_stop", comp_stop)
        ...
        summary = {
            ...
            "tests": [
                {"name": "test1", "profile": "anomaly-hot-partition", "duration_sec": self.args.hot_sec},
                {"name": "test2", "profile": "anomaly-compaction-pressure", "duration_sec": self.args.compaction_sec},
            ],
            "phase_durations": {
            ...
            "injection_profile": {
                ...
                "hot_threads": self.args.hot_threads,
                "hot_parallel_jobs": self.args.hot_parallel_jobs,
                "compaction_threads": self.args.compaction_threads,
                "compaction_pop_end": self.args.compaction_pop_end,
                "compaction_parallel_jobs": self.args.compaction_parallel_jobs,
            },
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Scenario B: bootstrap, then anomaly-hot-partition → cooldown → anomaly-compaction-pressure → cooldown."
    )
    ...
    parser.add_argument("--hot-sec", type=int, default=int(os.getenv("HOT_SEC", "150")))
    parser.add_argument("--cooldown1-sec", type=int, default=int(os.getenv("COOLDOWN1_SEC", "90")))
    parser.add_argument("--compaction-sec", type=int, default=int(os.getenv("COMPACTION_SEC", "180")))
    parser.add_argument("--cooldown2-sec", type=int, default=int(os.getenv("COOLDOWN2_SEC", "120")))
    ...
    # Hot partition tuning
    parser.add_argument("--hot-threads", type=int, default=None)
    parser.add_argument(
        "--hot-parallel-jobs",
        type=int,
        default=int(os.getenv("HOT_PARALLEL_JOBS", "8")),
        help="Parallel cassandra-stress Jobs for hot partition anomaly (default 8 for demo visibility).",
    )

    # Compaction tuning
    parser.add_argument("--compaction-threads", type=int, default=None)
    parser.add_argument("--compaction-pop-end", type=int, default=None)
    parser.add_argument(
        "--compaction-parallel-jobs",
        type=int,
        default=int(os.getenv("COMPACTION_PARALLEL_JOBS", "8")),
        help="Parallel cassandra-stress Jobs for compaction pressure anomaly (default 8 for demo visibility).",
    )
