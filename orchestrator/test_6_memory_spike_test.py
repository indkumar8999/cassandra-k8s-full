...
class ScenarioRunner:
    def __init__(self, args):
        ...
        self.run_id = args.run_id or f"test6-{int(self.run_started_at)}"
    ...
    def run(self):
        profile = "anomaly-mem-mixed-cached"

        self._record(
            "run_start",
            {
                "run_id": self.run_id,
                "test": "test_6_memory_spike_test",
                "fault_profile": profile,
                "load_profile": self.args.load_profile,
                "duration_sec": self.args.mem_sec,
                "note": "Runs anomaly-mem-mixed-cached (large rows + small working set) while learner phase is forced to chaos.",
            },
        )
        ...
        self._sleep_phase(Phase("chaos", self.args.mem_sec))
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Test 6: run anomaly-mem-mixed-cached (large rows + small working set) in chaos mode to bias toward Cassandra memory pressure."
    )
    ...
    parser.add_argument(
        "--mem-sec",
        type=int,
        default=int(os.getenv("MEM_SEC", os.getenv("CHAOS_SEC", "180"))),
        help="Duration of the mem-mixed-cached window (seconds).",
    )
...
def main():
    ...
    try:
        ...
        print(f"[ERROR] test_6_memory_spike_test failed: {ex}", file=sys.stderr)
        raise
