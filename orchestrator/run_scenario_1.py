...
class ScenarioRunner:
    ...
    def _concurrency_spike_body(self) -> Dict:
        body: Dict = {
            "profile": "anomaly-concurrency-spike",
            "duration_sec": self.args.concurrency_spike_duration,
        ...
        if self.args.stress_pop_end is not None:
            body["pop_end"] = self.args.stress_pop_end
    ...
    def run(self):
        ...
        # Test 1: baseline-normal in chaos (isolated; ideally no alarms)
        ...
        spike_body = self._concurrency_spike_body()
        ...
        self._sleep_phase(Phase("cooldown_final", self.args.cooldown_duration))
        ...
        summary = {
            ...
            "fault_profile": "anomaly-concurrency-spike",
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help="Recorded in run_summary for reporting (simulator /load is not driven by this script).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cassandra-stress threads= for anomaly-concurrency-spike /start_fault (omit for chaos-injector defaults).",
    )
    parser.add_argument(
        "--stress-pop-end",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Parallel cassandra-stress Jobs for anomaly-concurrency-spike (default matches run_scenario.py).",
    )
    parser.add_argument(
        "--skip-report",
        action="store_true",
        help="Do not run reporting/generate_report.py after the run.",
    )
    parser.add_argument(
