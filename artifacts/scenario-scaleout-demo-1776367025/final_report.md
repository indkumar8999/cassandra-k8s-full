# Cassandra UBL Run Report

## Run Metadata
- Run ID: `scenario-scaleout-demo-1776367025`
- Fault profile: `anomaly-concurrency-spike`
- Load profile: `high`
- Duration sec: `663.26`

## Fault Injection Profile (EDA)
- Target namespace: `cassandra-lab`
- CPU workers: `4`
- Memory MB: `2048`
- Bottleneck replicas: `2`
- Commanded chaos window: `{'start_ts': 1776367208.8765981, 'stop_ts': 1776367628.8991191}`

## Detection Summary
- TP: `23`
- FP: `0`
- FN(proxy): `0`
- Precision: `1.0000`
- Recall: `1.0000`
- F1: `1.0000`

## Lead-Time / Delay
- First detection delay sec: `340.0458891391754`
- Mean detection delay sec: `350.62517717610234`
- Median detection delay sec: `346.1776146888733`

## SOM Runtime Metrics
- Training duration sec: `0.166`
- Bootstrap samples: `350`
- Avg scoring latency ms: `1.45`
- BMU coverage count: `89`
- Scored by phase: `{'chaos': 557, 'cooldown': 113, 'load': 0, 'normal': 0}`
- Dropped missing Tier A: `0`
- Dropped missing Tier B: `None`

## Run Quality Gate
- Passed: `True`
- Chaos scored samples: `557`
- Chaos min required: `50`
- Message: `ok`

## Acceptance Criteria
- Passed: `True`
- Max FP allowed: `10`
- tp_during_chaos: `True`
- lead_time_present: `True`
- chaos_sample_gate_passed: `True`
- fp_within_limit: `True`

## Top Cause Metrics
- `tier_a_cpu_usage_cores`: `16`
- `tier_a_disk_read_bytes_per_sec`: `7`
- `tier_a_memory_working_set_bytes`: `7`

## SOM artifacts (saved per run)
- Per-sample **normalized input vectors** are in `score_stream.json` under `input_vector` (same order as `feature_order` in `som_snapshot.json`).
- **SOM weights** (`weights`) and **area map** (`area_map`) are in `som_snapshot.json`.
- Snapshot JSON: `/Users/aumpandya/Documents/Projects/ads_project/cassandra/artifacts/scenario-scaleout-demo-1776367025/som_snapshot.json`

## Notes
- Precision/recall are run-level indicators intended for comparative ablations.
- FN is proxy-based (`1` if no alarm during chaos window; else `0`) for MVP comparability.